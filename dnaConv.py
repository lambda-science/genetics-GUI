...
class dnaConv(Frame):
	...
	def createPresentator(self):
		...
		Checkbutton(self.menuframe, text="Complementaire", variable=self.comp, command=self.updateSeq).grid(row=0, column=0)
		Checkbutton(self.menuframe, text="Inverse", variable=self.reverse, command=self.updateSeq).grid(row=0, column=1)
